...
<Card.Root class="mx-auto w-full max-w-2xl rounded-2xl border bg-background shadow-md">
	...
	<Card.Content class="space-y-6 px-6 py-4 text-sm">
		...
		{#if $executionStore?.executable}
			<div>
				...
				<pre
					class="max-h-60 overflow-x-auto whitespace-pre-wrap rounded bg-muted p-3 font-mono text-xs">
{$executionStore.executable.split('\n').slice(2).join('\n')}
				</pre>
			</div>
		{/if}
